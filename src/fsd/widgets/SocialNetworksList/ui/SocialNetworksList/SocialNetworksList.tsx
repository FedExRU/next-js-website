'use client'

import { SocialNetwork, useGetSocialNetworksQuery } from '@fsd/entities'
import { Typography } from '@fsd/shared'
import { Grid, Tooltip } from '@mui/material'

import { generateTooltipText } from '../../lib'
import { getStyles } from './styles'

const { styles } = getStyles()

export const SocialNetworksList: React.FC = () => {
  const { data, isLoading } = useGetSocialNetworksQuery()

  if (isLoading) {
    return (
      <Grid container spacing={3}>
        {Array.from({ length: 3 }).map((_, i) => (
          <Grid key={i} size="auto" sx={styles.flexWrapper}>
            <SocialNetwork skeleton skeletonColor="secondary" />
          </Grid>
        ))}
      </Grid>
    )
  }

  return (
    <Grid container spacing={3}>
      {data?.items.map(({ id, name, ...rest }) => (
        <Grid key={id} size="auto" sx={styles.flexWrapper}>
          <Tooltip
            arrow
            describeChild
            disableTouchListener
            placement="top"
            title={
              <Typography variant="caption2">
                {generateTooltipText(name)}
              </Typography>
            }
          >
            <SocialNetwork id={id} name={name} {...rest} />
          </Tooltip>
        </Grid>
      ))}
    </Grid>
  )
}
